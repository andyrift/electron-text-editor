  ...
  <div @click="() => { emit('click'); }" @mouseenter="hover = true" @mouseleave="hover = false"
    ...
    <div class="flex w-full">
      ...
      <button :class="hover ? 'opacity-30 hover:opacity-100' : 'opacity-0'" class=" ml-1 transition-opacity
        h-fit outline-none" @click="(e) => { e.stopPropagation(); emit('delete'); }">
      ...
      <div class="relative">
        ...
        <Transition name="fadequick">
          ...
          <div v-if="folderMenu" class="absolute right-0 z-20 bg-white p-2 shadow-unilg mt-2 rounded w-48"
...
import { Core, PageManager, PubSub } from '@core';
...
const changeFolder = async (e: Event, id: (number | null), create: boolean = false) => { 
  ...
  if (props.page) {
    ...
    else await pageManager.changeFolder(props.page.id, id);
  }
}
